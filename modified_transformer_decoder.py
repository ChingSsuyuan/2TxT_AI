...
class COCOImageCaptionDataset(Dataset):
    ...
    def calculate_keyword_match(self, generated_keywords, true_keywords):
        ...
        Args:
            generated_keywords: 生成的关键词列表
            true_keywords: 真实的关键词列表
...
def generate_captions(model, dataset, dataloader, device, num_samples=5):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # 提取真实标题和生成标题的关键词
            true_keywords = dataset.extract_keywords(true_caption, max_keywords=3)
            generated_keywords = dataset.extract_keywords(generated_caption, max_keywords=3)
